namespace StringsAndArrays
{
    class Program
    {
        ...
        static string ToLovercase(string name)
        {
            // Unfortunately no standard library method exists for converting to 'lovercase', so I hope lowercase will do.
            return name.ToLower();
        }
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"First: {helloWorld[0]}, last: {helloWorld[helloWorld.Length - 1]}");
            ...
            Console.WriteLine($"The names: {string.Join(", ", names)}");

            // 2.4 Reverse the order of the items in the names array and outpu the result
            names = names.Reverse().ToArray();
            Console.WriteLine($"The names, but reversed: {string.Join(", ", names)}");
            ...
            words = helloWorld.Replace(",", "").Replace("!", "").Split(' ').ToArray();

            // 1.3 Output the values of the words array
            Console.WriteLine($"The individual words in helloWorld are: [{string.Join(", ", words)}]");
        }
    }
}
